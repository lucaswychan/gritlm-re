...
We integrated `SIGReg` from [LeJEPA](https://arxiv.org/pdf/2511.08544) on top of InfoNCE (`loss = InfoNCE(p, q, n) + SIGReg(p, q, n)`, where `p`, `q`, and `n` are embeddings of the query, positive sample, and negative samples) to reduce the batch size required for preventing representaion collapse in embeddings. However, it cannot directly acts as a regularization. In LeJEPA, SIGReg is applied to unnormalized embeddings. But in text embedding training, InfoNCE almost always operates on L₂-normalized embeddings. See the fix in the blog post ["A Model Trained on 200M Samples Still Collapses — And One Constant Fixes It"](https://medium.com/@lucaswychan/a-model-trained-on-200m-samples-still-collapses-and-one-constant-fixes-it-2c00fc5a8ebc).
